...
function customAssign(...objects) {
  ...
  const funAss = Object.assign({}, ...objects);
  ...
  for (const num in funAss) {
    if (typeof funAss[num] === "number") {
      funAss[num] = String(funAss[num]);
    }
  }
  return funAss;
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const objectFull = Object.freeze(obj);
  // Перевіряємо, чи об'єкт "заморожено"
  if (Object.freeze(obj) === objectFull) {
    return objectFull;
  } else {
    return "Oб'єкт не було заморожено";
  }
...
function filterObjectsByKey(arr, key) {
  ...
  const nameKey = arr.filter((elem) => elem.hasOwnProperty(key));
...
function checkOwnership(obj1, obj2, key) {
  ...
  const ob1 = obj1.hasOwnProperty(key);
  const ob2 = obj2.hasOwnProperty(key);
  // Якщо обидва об'єкти мають вказаний ключ повертаємо результат порівняння їх значень.
  if (Object.is(ob1, ob2) === true) {
    return true;
  } else if (Object.is(ob1, ob2) !== true) {
    return false;
...
function getObjectValuesSum(obj) {
  ...
  const result = val.reduce((num, sum) => num + sum, 0);
  // Повертаємо суму
  return result;
}
...
function convertArrayToObj(arr) {
  ...
  let obj = {};
  // Проходимося по кожному підмаси ву в масиві за допопмогою циклу for, лічильник від нуля до довжини масиву
  for (let i = 0; i < arr.length; i++) {
    // Розпаковуємо підмасив за допомогою деструктурізації на окремі змінні для ключа та значення
    const [key, value] = arr[i];
    // Перевіряємо, чи існує вже ключ в об'єкті,якщо так виводимо
    //в консоль повідомлення `У масиві є дубльований ключ: ${key}`
